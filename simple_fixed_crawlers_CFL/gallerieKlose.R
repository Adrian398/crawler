GalerieKlose_crawler <- function(){
  ...
  klosedaten %>%
    read_html() %>%
    html_nodes(".centered~ p+ p .italicBold")%>%
    html_text() %>% 
  ...
  klosedaten %>%
    ...
    html_text() %T>% 
    print() %>%
    str_extract("[0-9]{1,2}( – |-| bis )[0-9]{1,2} Uhr") %T>% 
    print() %>%
    str_extract("[0-9]{1,2}( – |-| bis )") %T>% 
    print() %>%
    str_replace("( – |-| bis )", ".00") %>% 
  ...
  klose_datum = klose_datum[1:length(klose_titel)]
  klose_zeit_bis = klose_zeit_bis[1:length(klose_titel)]
  klose_zeit = klose_zeit[1:length(klose_titel)]
